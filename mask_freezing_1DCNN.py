...
t_s_list = [20,30,40,50] # Time steps
...
def run_test():
    ...
    class Masked1DCNN(nn.Module):
        ...
        Matches Algorithm B (activation-level masking):
        ...
        def set_mask(self, mask):
            # Optional debug only (forward() uses the mask argument).
            self.mask = mask
    ...
    for t_w in t_w_list:
        ...
        for t_dim in t_s_list:
            baseline_res = None # For computing p-values to determine statistically significant values
            for d_s in d_s_list:
                ...
                for fold, (train_index, val_index) in enumerate(kf.split(inputs_tensor, targets_tensor)):
                    ...
                    for j, seed_j in enumerate(seed_values):
                        ...
                        for epoch in range(num_epochs):
                            ...
                            if save_baseline_model and d_s == 0:
                                # Save baseline model
                                torch.save(model.state_dict(), os.path.join(parent_dir,model_dir,f'baseline_model_{t_w}tw_{t_dim}ts_{d_s}ds_{fold}k_{seed_j}seed_{epoch}epoch_{start_time_file_readable}.pth'))
                            
                            if save_all_models and d_s != 0:
                                # Save baseline model
                                torch.save(model.state_dict(), os.path.join(parent_dir,model_dir,f'curr_model_{t_w}tw_{t_dim}ts_{d_s}ds_{fold}k_{seed_j}seed_{epoch}epoch_{start_time_file_readable}.pth'))
                            ...
                            if curr_score > best_overall_score:
                                ...
                                if save_best_model and d_s != 0:
                                    # Save the best model so far
                                    torch.save(model.state_dict(), os.path.join(parent_dir,model_dir,f'best_model_{t_w}tw_{t_dim}ts_{d_s}ds_{fold}k_{seed_j}seed_{epoch}epoch_{start_time_file_readable}.pth'))
